 ...
 * storage in arbitrary nests. The scalpel has been going around long enough
 * that you may assume that every nest has a "scalpel" entry in its data
 * storage.
 * 
...
async function locateScalpel(nest) {
  let scalpel = nest.name;
  while (scalpel != nest.name) {
    scalpel = await anyStorage(nest, scalpel, "scalpel");
  }
  return scalpel;
}

function locateScalpel2(nest) {
  function findScalpel(current) {
